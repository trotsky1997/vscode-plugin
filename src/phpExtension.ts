...
export async function activatePhp(context: vscode.ExtensionContext) {
    ...
    async function _activate() {
        ...
        if (msphp) {
            ...
            hooked = await JSHooker("-online-1**/", distjsPath, msphp, "php.reload.msphp", "php.fail.msphp", (distjs) => {
        ...
        if (intelephense) {
            ...
            intelephenseHooked = await JSHooker("/**AiXHooked-local-3**/", distjsPath, intelephense, "php.reload.intelephense", "php.fail.intelephense", (distjs) => {
    ...
    const provider = {
        async provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken, completionContext: vscode.CompletionContext): Promise<vscode.CompletionItem[] | vscode.CompletionList> {
            ...
            try {
                ...
                if ((msphp && msphp.isActive && hooked) || (intelephense && intelephense.isActive && intelephenseHooked)) {
